...
def process_dataframe(df):
    ...
    text_columns_for_features = [
        '學校關注事項', '學習和教學策略', '小學教育課程更新重點的發展', 
        '共通能力的培養', '正確價值觀、態度和行為的培養', '全校參與照顧學生的多樣性',
        '全校參與模式融合教育', '非華語學生的教育支援', '課程剪裁及調適措施',
        '家校合作', '校風', '學校發展計劃', '教師專業培訓及發展', '其他未來發展',
        '辦學宗旨', '全方位學習', '特別室', '其他學校設施'
    ]
    ...
    # 師資百分比處理
    percentage_cols = [
        '已接受師資培訓(佔全校教師人數%)', '學士(佔全校教師人數%)', '碩士、博士或以上 (佔全校教師人數%)', 
        '特殊教育培訓 (佔全校教師人數%)', '0-4年資 (佔全校教師人數%)', '5-9年資(佔全校教師人數%)', 
        '10年或以上年資 (佔全校教師人數%)'
    ]
...
if uploaded_file is not None:
    try:
        ...
        with st.expander("📍 按地區及校網搜尋", expanded=True):
            ...
            with col1:
                all_districts = sorted(processed_df['地區'].dropna().unique())
                selected_districts = st.multiselect("**選擇地區 (可多選)**", options=all_districts)
        ...
        with st.expander("🌟 按辦學特色搜尋", expanded=False):
            ...
            with col1:
                selected1 = st.multiselect("教學模式與重點", options=list(feature_mapping["【教學模式與重點】"].keys()))
                all_selected_options.extend(selected1)
            with col2:
                selected2 = st.multiselect("價值觀與品德", options=list(feature_mapping["【價值觀與品德】"].keys()))
                all_selected_options.extend(selected2)
            with col3:
                selected3 = st.multiselect("學生支援與發展", options=list(feature_mapping["【學生支援與發展】"].keys()))
                all_selected_options.extend(selected3)
        ...
        for index, school in filtered_df.iterrows():
            with st.expander(f"**{school['學校名稱']}** ({school.get('地區', 'N/A')})"):
                ...
                st.markdown("---")
                total_teachers = school.get('全校教師總人數', 0); approved_teachers = school.get('核准編制教師職位數目', 0); diff = total_teachers - approved_teachers
                st.markdown("#### 🧑‍🏫 師資團隊概覽")
                col1, col2 = st.columns(2)
